...
export default function Header({ search, onSearch, dark, onToggleDark }: HeaderProps) {
  ...
  const userInitials = isGuest ? 'G' : (currentUser?.name?.split(' ').map(n => n[0]).join('').toUpperCase() || 'U')
  ...
  useEffect(() => {
    // This ensures the component re-renders with the latest user data
  }, [currentUser, isGuest])
          ...
          {showAvatar && (
            <div className="dropdown-panel avatar-panel">
              <div className="avatar-panel-profile">
                <div className="avatar-panel-icon">{userInitials}</div>
                <div>
                  <p className="avatar-panel-name">{userName}</p>
                  <p className="avatar-panel-role">{isGuest ? 'Guest' : 'Student'}</p>
                </div>
              </div>
              ...
              <button className="avatar-panel-item logout" onClick={handleLogout}>
